...
ingredients_client = chromadb.PersistentClient(path="DBScript/fairprice_openai_embeddings_db")
...
def process_selected_recipe(query_text, selected_recipe):
    ...
    # Query ingredients from ChromaDB embeddings
    ingredients_from_db = {
    ...
    return {
        ...
        "contexts": [
            ...
            f"FairPrice Ingredient: {prod['metadata']['name']} by {prod['metadata']['brand']} "
            f"(Price: ${prod['metadata']['price']}, Size: {prod['metadata']['size']}, "
            f"URL: {prod['metadata'].get('url', 'N/A')})"
            for ing, prods in ingredients_from_db.items()
            for prod in prods
            if prod['metadata'].get('url', 'N/A') == 'N/A' or
               is_valid_url(prod['metadata'].get('url', 'N/A'))
        ]
    }

if __name__ == "__main__":
    # This call will fail unless you define 'query_all'
    # or change it to your actual function name (e.g., get_recipe_choices)
    query_text = "cheap high protein tofu dish"
    result = query_all(query_text)  # <--- 'query_all' is not defined in this script
